...
const HeroSafe = async () => {
  const normStats = await getNormStats();
  const t = await getT();
  return (
    <section className="relative pt-32 pb-20 px-6 overflow-hidden">
      {/* Background decoration */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-gradient-to-b from-primary/20 to-transparent rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative max-w-5xl mx-auto text-center">
        ...
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight mb-6">
          {t("hero.heading")}{" "}
          <span className="text-gradient-safe">{t("hero.headingHighlight")}</span>
        </h1>

        {/* Subheading - Differentiation message */}
        <p className="text-lg md:text-xl text-base-content/70 max-w-2xl mx-auto mb-4">
          {t("hero.subheading")}{" "}
          <span className="font-semibold text-base-content">{t("hero.subheadingBold")}</span>
        </p>

        {/* Anti-audit message */}
        <p className="text-base text-base-content/50 max-w-xl mx-auto mb-10">
          {t("hero.auditStat")}{" "}
          <span className="text-primary font-medium">{t("hero.realSecurity")}</span>
        </p>

        {/* CTA buttons */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8">
          <Link href="/dashboard/setups" className="btn btn-primary btn-lg gap-2 animate-pulse hover:animate-none">
        ...
        <div className="mt-16 flex flex-wrap items-center justify-center gap-8 text-base-content/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">{t("hero.independentScores")}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">{t("hero.productTypes", { count: normStats?.totalProductTypes ?? "—" })}</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">{t("hero.openMethodology")}</span>
          </div>
        </div>
      </div>
    </section>
