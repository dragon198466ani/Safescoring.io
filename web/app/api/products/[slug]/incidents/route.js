...
export async function GET(request, { params }) {
  try {
    ...
    // Try static JSON first (zero DB cost, pre-generated by build script)
    try {
      const staticPath = path.join(process.cwd(), "public", "data", "incidents", `${slug}.json`);
      ...
      // Static file read failed, fall through to Supabase
    }

    // Fallback: Supabase query (for products without static data)
    if (!isSupabaseConfigured()) {
      return NextResponse.json(
        { incidents: [], stats: { totalIncidents: 0, totalFundsLost: 0, bySeverity: {}, riskLevel: "low" } }
      );
    }
    ...
    const incidentsData = (impactData || [])
      ...
      .map(item => ({
        ...
        funds_recovered: item.security_incidents.funds_recovered_usd || 0,
        ...
        source_urls: item.security_incidents.source_urls || [],
        impact_level: item.impact_level,
        chain: item.security_incidents.chain || null,
        resolved_date: item.security_incidents.resolved_date || null,
        postmortem_url: item.security_incidents.postmortem_url || null,
        discovered_date: item.security_incidents.discovered_date || null,
      }))
    ...
    const incidents = (incidentsData || []).map((incident) => {
      ...
      return {
        ...
        date: incident.date,
        dateIsEstimated: incident.date_is_estimated || false,
        fundsLost,
        fundsRecovered,
