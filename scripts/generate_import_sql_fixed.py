...
for _, row in df.iterrows():
    ...
    title = str(row['Norme']).strip()[:200].replace("'", "''") if pd.notna(row['Norme']) else code
    ...
    access = str(row['Accès']).strip()[:50].replace("'", "''") if pd.notna(row['Accès']) else None
    source = str(row['Source']).strip()[:100].replace("'", "''") if pd.notna(row['Source']) else None
...
# Split into batches of 50 for smaller queries
batch_size = 50
